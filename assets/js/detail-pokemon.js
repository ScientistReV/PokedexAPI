...
function loadPokemonInfo(id) {
    pokeApi.getPokemonInfo(id).then((pokemon) => {
        const newHtml = `
            <div class="col pokemonHeader ${pokemon.pokemonType}">
                <div class="header">
            ...
            <div class="col pokemonStats">
                ...
                <div id="Information" class="tabcontent">
                    <label>Weight:</label> 
                    <p class="infoText">${pokemon.pokemonWeight/10}Kg</p>

                    <label>Height:</label>
                    <p class="infoText">${pokemon.pokemonHeight/10}m</p>

                    <h2>Abilities</h2>

                    <ol class="abilities">
                        ${pokemon.pokemonAbilities.map((ability) => `<li class="ability">${ability}</li>`).join('')}
                    </ol>
                </div>
                ...
                <ol class="stats">
                    ...
                    <li class="stat">${stat.split('|')[0].replace('-', ' ')}
                        <div class="bar">
                            <div class="skills ${pokemon.pokemonType}" style="max-width: ${stat.split('|')[1]}%; ">${stat.split('|')[1]}</div>
                        </div>
                    </li>`).join('')}
                </ol> 
                </div>         
